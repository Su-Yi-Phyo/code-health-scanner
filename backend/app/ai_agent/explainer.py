...
from ai_agent.bob_provider import explain_with_bob  # noqa: E402
...
def explain_issues(
    ...
    Provider order
    --------------
    1. IBM Bob Shell  -- tried first when BOB_API_KEY is in the environment.
    2. Hugging Face   -- used when Bob is unavailable or fails.
    ...
    """
    # --- Try IBM Bob first --------------------------------------------------
    bob_result = explain_with_bob(file_path, language, source_code, duplicates, dead_code)
    if bob_result.get("_success"):
        return {k: v for k, v in bob_result.items() if k != "_success"}
